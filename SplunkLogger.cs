...
namespace SplunkClient
{
    ...
    * for developers logging mobile data to Splunk */
    public class SplunkLogger
    {
        ...
        /*
        * Constructs a SplunkLogger with endpoint URI, Http Event Collector token, bool SSL */
        public SplunkLogger(string newUri, string token, bool ssl)
        {
            ...
            // Adds severity levels for SplunkLogger
        ...
        * Changes the sourcetype of this SplunkLogger */
        ...
        /*
        * Logs a string message/event to Splunk's Http Event Collector, Sync */ 
        ...
        * Collector, Sync or Async depending on the passed bool */
        ...
        /*
        * Handles the logic of making and sending event batches by size, Async */
        async private Task HandleBatching(string message)
        {
            eventBatch.Enqueue(message);
            if (eventBatch.Count == batchSize)
            {
                await SendAsync(eventBatch);
            }
        }

        /*
        * Handles batching by time interval, runs in background, Async */
        ...
        /*
        * Sends events to this Logger's URI,
        * events queue is empty upon completion, Async */
        async public Task SendAsync(Queue<string> events)
        {
            string JSONstr = "";
            while (events.Count > 0)
            {
                string curMessage = events.Dequeue();
        ...
        * Returns a JSON populated HttpContent object with the given message */
        ...
        /*
        * Resends all events that caused exceptions, Async */
